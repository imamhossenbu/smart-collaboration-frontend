...
export default function EditProjectModal({
  ...
  const onSubmit = async (data: any) => {
    ...
    await onUpdate(project.id, formattedData);
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden">
        ...
        <form
          ...
          className="px-8 pb-8 pt-5 space-y-5 max-h-[70vh] overflow-y-auto"
        >
          <input
            {...register("name")}
            className="w-full px-5 py-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-slate-200"
            placeholder="Project Name"
          />

          <textarea
            {...register("description")}
            className="w-full px-5 py-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-slate-200 h-24"
